...
Note: `Analysis.make_prediction` utilizes a fast bulk prediction operation supported by many Fero models. For some legacy blueprints, you may use `Analysis.make_prediction_serial` to fetch predictions one row at a time. Similarly, if you are making predictions for very large inputs and your network is unable to send and fetch complete dataframes, `Analysis.make_prediction_serial` will fetch predictions in smaller increments.
